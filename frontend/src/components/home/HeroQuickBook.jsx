...
import { createBooking, getSlots } from '../../api/client'
import {
...
const inputClass =
  'h-full w-full min-w-0 bg-transparent py-3 pl-3 text-sm text-white outline-none placeholder:text-muted'
...
export default function HeroQuickBook() {
  ...
  useEffect(() => {
    ...
    const load = () => {
      getSlots(date)
        .then((data) => {
          ...
          setSelectedTimes((prev) => prev.filter((t) => !taken.includes(t)))
        })
        .catch(() => {
    ...
    window.addEventListener('shree:bookings-changed', onChanged)
    return () => {
      cancelled = true
      window.removeEventListener('shree:bookings-changed', onChanged)
    }
  }, [date])
  ...
  const handleCheck = async (e) => {
    ...
    if (!date || selectedTimes.length === 0 || !game || !customerName.trim() || !phone.trim()) {
      setError('Please fill date, time, game, name, and phone')
      return
    }
    setError('')
    setTimeOpen(false)
          ...
          <AnimatePresence mode="wait">
            {status === 'available' ? (
              ...
              >
                            ...
                            <button
                              ...
                              className={cn(
                                ...
                                taken
                                  ...
                                  : 'hover:bg-primary/10',
                                selected ? 'bg-primary/10 text-primary' : !taken && 'text-white',
                              )}
                            >
                              <span>
                                {slot}
                ...
                <label className={fieldShell}>
                  <FaUser className="pointer-events-none shrink-0 text-muted" size={14} />
                  <input
                    type="text"
                    value={customerName}
                ...
                <label className={fieldShell}>
                  <FaPhone className="pointer-events-none shrink-0 text-muted" size={14} />
                  <input
                    type="tel"
                    value={phone}
                ...
                >
                  {status === 'checking' ? 'Booking…' : 'Book Slot'}
                </motion.button>
              </motion.form>
            )}
          </AnimatePresence>
